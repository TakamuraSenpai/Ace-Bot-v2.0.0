...
const { pornopremium } = require('./src/pornopremium')
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Takamura\n' // Nama kamu
            + 'ORG:Ace;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=558695023910:+55 86 9502 3910\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '/','*','.'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Escaneie Codigo QR Code ai Patrao'))
	})
	...
	client.on('open', () => {
		success('2', 'Conectado Com Sucesso By:Takamura')
	})
		...
		try {
			...
			if (anu.action == 'add') {
				...
				try {
					...
					ppimg = 'https://hdwallpaperim.com/wp-content/uploads/2017/08/23/104345-One_Piece-Portgas_D._Ace.jpg'
				}
				...
				teks = `𝒪𝓅𝒶 𝒱𝒶𝒾 𝒫𝑒𝓁𝒶 𝒮𝑜𝓂𝒷𝓇𝒶 @${num.split('@')[0]} 😣✋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '❬ッ❭ 𝒞𝒶𝓁𝓂𝒶 𝒶𝒾 𝐸𝓈𝓉𝑜𝓊 𝒫𝓇𝑜𝒸𝑒𝓈𝓈𝒶𝓇 𝑜 𝒞𝑜𝓂𝒶𝓃𝒹𝑜...',
					success: '️❬ ✔ ❭ Sucesso 🖤',
					levelon: '❬ ✔ ❭ *leveling* *𝒞𝑜𝓂𝒶𝓃𝒹𝑜 "𝒩𝒾𝓋𝑒𝓁" 𝐹𝑜𝒾 𝒜𝓉𝒾𝓋𝒶𝒹𝑜 𝓃𝑜 𝒢𝓇𝓊𝓅𝑜...*',
					leveloff: ' ❬ X ❭  *leveling* *𝒞𝑜𝓂𝒶𝓃𝒹𝑜 "𝒩𝒾𝓋𝑒𝓁" 𝐹𝑜𝒾 𝒟𝑒𝓈𝒶𝓉𝒾𝓋𝒶𝒹𝑜 𝓃𝑜 𝒢𝓇𝓊𝓅𝑜...*',
					levelnoton: '❬ X ❭ *𝒞𝑜𝓂𝒶𝓃𝒹𝑜 𝒩𝒾𝓋𝑒𝓁 𝒩𝒶𝑜 𝐹𝑜𝒾 𝒜𝓉𝒾𝓋𝒶𝒹𝑜 𝓃𝑜 𝒢𝓇𝓊𝓅𝑜...*',
					levelnol: '*error* 0 °-°',
					error: {
				...
				only: {
					group: '[†] 𝐸𝓈𝓉𝑒 𝒸𝑜𝓂𝒶𝓃𝒹𝑜 𝓈𝑜 𝓅𝑜𝒹𝑒 𝓈𝑒𝓇 𝓊𝓈𝒶𝒹𝑜 𝑒𝓂 𝑔𝓇𝓊𝓅𝑜𝓈! ❌',
					premium: '[❗] 𝐸𝓈𝓉𝑒 𝒞𝑜𝓂𝒶𝓃𝒹𝑜 𝐸 𝒜𝓅𝑒𝓃𝒶𝓈 𝒫𝒶𝓇𝒶 𝒰𝓈𝓊𝒶𝓇𝒾𝑜𝓈 *PREMIUMS*',
					mod: '[❗] 𝐸𝒮𝒯𝐸 𝒫𝐸𝒟𝐼𝒟𝒪 𝐸 𝐸𝒮𝒫𝐸𝒞𝐼𝐹𝐼𝒞𝒪 𝒫𝒜𝑅𝒜 𝒰𝒮𝒰𝒜𝑅𝐼𝒪 𝑀𝒪𝒟 𝒜𝒞𝐸 𝐵𝒪𝒯*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Proprietário? 𝐸𝓈𝓉𝑒 𝑒 𝓊𝓂 𝓇𝑒𝒸𝓊𝓇𝓈𝑜 𝑒𝓈𝓅𝑒𝒸𝒾𝒶𝓁 𝓅𝒶𝓇𝒶 𝑜 𝓅𝓇𝑜𝓅𝓇𝒾𝑒𝓉𝒶𝓇𝒾𝑜 𝒹𝑜 𝒷𝑜𝓉 ❌',
					ownerB: '[❗] Proprietário? 𝐸𝓈𝓉𝑒 𝑒 𝓊𝓂 𝓇𝑒𝒸𝓊𝓇𝓈𝑜 𝑒𝓈𝓅𝑒𝒸𝒾𝒶𝓁 𝓅𝒶𝓇𝒶 𝑜 𝓅𝓇𝑜𝓅𝓇𝒾𝑒𝓉𝒶𝓇𝒾𝑜 𝒹𝑜 𝒷𝑜𝓉 ❌',
					...
					admin: '[†] 𝐸𝓈𝓉𝑒 𝒸𝑜𝓂𝒶𝓃𝒹𝑜 𝓈𝑜 𝓅𝑜𝒹𝑒 𝓈𝑒𝓇 𝓊𝓈𝒶𝒹𝑜 𝓅𝑜𝓇 𝒶𝒹𝓂𝒾𝓃𝒾𝓈𝓉𝓇𝒶𝒹𝑜𝓇𝑒𝓈 𝒹𝑒 𝑔𝓇𝓊𝓅𝑜! ❌',
					Badmin: ' [†] 𝐸𝓈𝓉𝑒 𝒸𝑜𝓂𝒶𝓃𝒹𝑜 𝓈𝑜 𝓅𝑜𝒹𝑒 𝓈𝑒𝓇 𝓊𝓈𝒶𝒹𝑜 𝓆𝓊𝒶𝓃𝒹𝑜 𝑜 𝒷𝑜𝓉 𝓈𝑒 𝓉𝑜𝓇𝓃𝒶 𝒶𝒹𝓂𝒾𝓃𝒾𝓈𝓉𝓇𝒶𝒹𝑜𝓇! ❌',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["558695023910@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"558695023910@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["558695023910@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["55@s.whatsapp.net"]//ubah nomor lo
			const premium = ["558695023910@s.whatsapp.net","558695023910@s.whatsapp.net","557999914319@s.whatsapp.net","351967803137@s.whatsapp.net"]
...
axios.get(`https://www.receitaws.com.br/v1/cnpj/${aris}`).then((res) => {
	conn.sendMessage(id, '[❗] 𝐸𝒮𝒫𝐸𝑅𝐸 𝐸𝒮𝒯𝒪𝒰 𝐵𝒰𝒮𝒞𝒜𝒩𝒟𝒪 𝒟𝒜𝒟𝒪𝒮', MessageType.text)
...
axios.get(`http://geradorapp.com/api/v1/cpf/generate?token=40849779ec68f8351995def08ff1e2fa`).then((res) => {
	conn.sendMessage(id, '[❗] 𝐸𝒮𝒫𝐸𝑅𝐸 𝐸𝒮𝒯𝒜 𝒫𝑅𝒪𝒞𝐸𝒮𝒮𝒜𝒩𝒟𝒪', MessageType.text)
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	client.sendMessage(from, tujuh, from , putagg ,image , MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
			...
			switch(command) {
				...
				case 'menu1':
				    ...
				    try {    
						...
						sa = await getBuffer(`https://iphone-wallpaper-hd.art/wp-content/uploads/5674.jpg`)
          ...
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'modoanime':
					...
					if ((args[0]) === 'on') {
						if (isAnime) return reply('ᴏ ᴍᴏᴅᴏ ᴀɴɪᴍᴇ ᴊᴀ ᴇsᴛá ᴀᴛɪᴠᴏ')
						anime.push(from)
						fs.writeFileSync('./database/json/anime.json', JSON.stringify(anime))
						reply(`\`\`\`✓ᴀᴛɪᴠᴀᴅᴏ ᴄᴏᴍ sᴜᴄᴇssᴏ ᴏ ᴍᴏᴅᴏ ᴀɴɪᴍᴇ ɴᴏ ɢʀᴜᴘᴏ\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						anime.splice(from, 1)
						fs.writeFileSync('./database/json/anime.json', JSON.stringify(anime))
						reply(`\`\`\`✓ᴍᴏᴅᴏ ᴀɴɪᴍᴇ ᴅᴇsᴀᴛɪᴠᴀᴅᴏ ᴄᴏᴍ sᴜᴄᴇssᴏ ɴᴏ ɢʀᴜᴘᴏ\`\`\` *${groupMetadata.subject}*`)
					} else {
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`ᴍᴏᴅᴏ ᴀɴᴛɪ ʀᴀᴄɪsᴍᴏs ᴏɴ ɴᴏ ɢʀᴜᴘᴏ:\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						antiracismo.splice(from, 1)
						fs.writeFileSync('./database/json/antiracismo.json', JSON.stringify(antiracismo))
						reply(`\`\`\`✓ᴍᴏᴅᴏ ᴀɴᴛɪʀᴀᴄɪsᴍᴏ ᴅᴇsᴀᴛɪᴠᴀᴅᴏ ᴄᴏᴍ sᴜᴄᴇssᴏ ɴᴏ ɢʀᴜᴘᴏ\`\`\` *${groupMetadata.subject}*`)
					} else {
						reply('On para ativar, Off para desligar')
					}
					break
				case 'modonsfw':
					...
					if (args.length < 1) return reply('Hmmmm')
					if ((args[0]) === 'on') {
						if (isNsfw) return reply('ᴏ ᴍᴏᴅᴏ ɴsғᴡ ᴊᴀ ᴇsᴛá ᴀᴛɪᴠᴏ')
						nsfw.push(from)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply(`\`\`\`✓ᴀᴛɪᴠᴀᴅᴏ ᴄᴏᴍ sᴜᴄᴇssᴏ ᴏ ᴍᴏᴅᴏ ɴsғᴡ ɴᴏ ɢʀᴜᴘᴏ\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply(`\`\`\`✓ᴍᴏᴅᴏ ɴsғᴡ ᴅᴇsᴀᴛɪᴠᴀᴅᴏ ᴄᴏᴍ sᴜᴄᴇssᴏ ɴᴏ ɢʀᴜᴘᴏ\`\`\` *${groupMetadata.subject}*`)
					} else {
						reply('On para ativar, Off para desligar')
					}
					break
		...
		case 'bpfont':
			...
			break 
				...
				uptime = process.uptime()
                    client.sendMessage(from, `Velocidade Do Bot de Resposta: *${latensi.toFixed(4)} _Segundos_*\nDispositivo: *PC*\nRAM: *16*\nData: *Telkomsel*\nRede: *Wifi*\nStatus: *Ainda não cobrado*\nTipo do bot: *Termux Somente*\n\n*O bot esteve ativo por*\n*${kyun(uptime)}*`, text, { quoted: mek})
                    break
				case 'delete':
				case 'del':
				case 'd':  
					if (!isPremium) return reply('ᴠᴏᴄᴇ ɴᴀᴏ ᴇ ᴜᴍ ᴍᴇᴍʙʀᴏ ᴘʀᴇᴍɪᴜᴍ, ᴇɴᴛʀᴇ ᴇᴍ ᴄᴏɴᴛᴀᴛᴏ ᴄᴏᴍ ᴏ ᴘʀᴏᴘʀɪᴇᴛᴀʀɪᴏ ᴏᴜ ᴅɪɢɪᴛᴇ *.Daftarvip* ᴘᴀʀᴀ ᴀᴅǫᴜɪʀɪʀ ᴏ ᴀᴄᴇssᴏ ᴘʀᴇᴍɪᴜᴍ!' ,text, { quoted: mek })
					if (!isGroup)return reply(mess.only.group)
					if (!isGroupAdmins)return reply(mess.only.admin)
                ...
                case 'travar':
                     const pesan = body.slice(5)
                      ...
                      if (pesan.length > 5000) return client.sendMessage(from, 'ate 5k de caracteres', msgType.text, {quoted: mek})
                       ...
                       const teks1 = `*[TRAVA(ಠ_ಠ)]*\nTRAVADO POR : @${nomor.split("@s.whatsapp.net")[0]}\nMensagem : ${pesan}`
                       const teks2 = `*[TRAVA(ಠ_ಠ)]*\nTRAVADO POR : @${nomor.split("@s.whatsapp.net")[0]}\nMensagem : ${pesan}`
                       const teks3 = `*[TRAVA(ಠ_ಠ)]*\nTRAVADO POR : @${nomor.split("@s.whatsapp.net")[0]}\nMensagem : ${pesan}`
                       const teks4 = `*[TRAVA(ಠ_ಠ)]*\nTRAVADO POR : @${nomor.split("@s.whatsapp.net")[0]}\nMensagem : ${pesan}`
                      var options = {
                         text: teks1,
                         text: teks2,
                         text: teks3,
...
// recursos anime, random @nabuto domina vida
          ...
          reply('Espere..')
			    ...
			    case 'dono':
                    ...
                    client.sendMessage(from, 'Meu Dono e o "Takamura SenPai" Antigo Samurai Aposentado"',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'bot':
			     	memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://static.zerochan.net/Portgas.D..Ace.full.1021188.jpg`)
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Takamura\n*Youtuber:* https://www.youtube.com/watch?v=N-y3c9wrwnw\n*WPP:* wa.me/++8695023910\n*\n\nEspero que tenham gostado do bot️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
        ...
        case 'promover':
				...
				case 'bemvindo':
					...
					if (Number(args[0]) === 1) {
						...
						reply('ᴀᴛɪᴠᴏᴜ ᴄᴏᴍ sᴜᴄᴇssᴏ ᴏ ʀᴇᴄᴜʀsᴏ ᴅᴇ ʙᴏᴀs-ᴠɪɴᴅᴀs ɴᴇsᴛᴇ ɢʀᴜᴘᴏ 😉️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('ᴅᴇsᴀᴛɪᴠᴏᴜ ᴄᴏᴍ sᴜᴄᴇssᴏ ᴏ ʀᴇᴄᴜʀsᴏ ᴅᴇ ʙᴏᴀs-ᴠɪɴᴅᴀs ɴᴇsᴛᴇ ɢʀᴜᴘᴏ 😡️')
					} else {
